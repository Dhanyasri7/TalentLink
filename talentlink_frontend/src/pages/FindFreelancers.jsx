...
import styles from "../styles/FindProjects.module.css"; // same theme
...
function FindFreelancers() {
  ...
  // Fetch freelancers from backend with optional filters
  const fetchFreelancers = async (appliedFilters = filters) => {
    try {
      let query = [];
      if (appliedFilters.skills) query.push(`skills__icontains=${appliedFilters.skills}`);
      if (appliedFilters.minRate) query.push(`hourly_rate__gte=${appliedFilters.minRate}`);
      if (appliedFilters.maxRate) query.push(`hourly_rate__lte=${appliedFilters.maxRate}`);
      if (appliedFilters.availability)
        query.push(`availability=${appliedFilters.availability === "true" ? true : false}`);
  ...
  const handleFilter = (e) => {
    e.preventDefault();
    fetchFreelancers(); // fetch with updated filters
  };
        ...
        {freelancers.length === 0 ? (
          ...
          freelancers.map((freelancer) => (
            <div key={freelancer.id} className={styles.card}>
              ...
              <p>Availability: {freelancer.availability ? "Available" : "Not Available"}</p>
              <p>Portfolio: {freelancer.portfolio || "No portfolio provided"}</p>
            </div>
          ))
        )}
      </div>
